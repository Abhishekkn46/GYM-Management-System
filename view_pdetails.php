...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT r.*,p.plan,p.amount as pamount,pp.package,pp.amount as ppamount,concat(m.firstname,' ',m.middlename,' ',m.lastname) as name,m.member_id as mid_no FROM registration_info r INNER JOIN members m ON m.id = r.member_id INNER JOIN plans p ON p.id = r.plan_id INNER JOIN packages pp ON pp.id = r.package_id WHERE r.id=".$_GET['id'])->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}

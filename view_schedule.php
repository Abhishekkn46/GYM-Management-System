...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT s.*,concat(m.firstname,' ', m.middlename,' ',m.lastname) as name FROM schedules s inner join members m on m.id = s.member_id where s.id=".$_GET['id'])->fetch_array();
	foreach($qry as $k =>$v){
		$$k = $v;
	}
}

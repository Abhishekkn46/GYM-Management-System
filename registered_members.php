					...
					<div class="card-body">
						...
						<table class="table table-bordered table-condensed table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$member =  $conn->query("SELECT r.*,p.plan,pp.package,concat(m.firstname,' ',m.middlename,' ',m.lastname) as name,m.member_id from registration_info r inner join members m on m.id = r.member_id inner join plans p on p.id = r.plan_id inner join packages pp on pp.id = r.package_id where r.status = 1 order by r.id asc");
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-outline-primary view_member" type="button" data-id="<?php echo $row['id'] ?>" >Viewa</button>

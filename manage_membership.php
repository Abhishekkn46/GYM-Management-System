		...
		<div id="msg"></div>
			<div class="form-group">
				...
				<select name="member_id" required="required" class="custom-select select2" id="">
					<option value=""></option>
					<?php
						$qry = $conn->query("SELECT *,concat(firstname,' ',middlename,' ',lastname) as name FROM members where id not in (SELECT member_id from registration_info where status = 1) order by concat(firstname,' ',middlename,lastname) asc");
